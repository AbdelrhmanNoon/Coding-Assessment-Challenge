        ...
        ScrollView {
            LazyVStack(alignment: .leading) {
                ForEach(viewModel.filteredCountries, id: \.id) { country in
                    Button(action: {
                        coordinator?.routeToCountryDetails(countryDetails: country)
                    }) {
                        CountryBannerView(capitalname: country.capital ?? "",
                                          countryName: country.name, flagAvatar: country.flags?.png ?? "")
